<%@ page language="C#" autoeventwireup="true" CodeFile="Default8b.aspx.cs" Inherits="Default8" MaintainScrollPositionOnPostback="true"%>
...
<body>
    ...
    <HeaderStyle BackColor="#000084" Font-Bold="True" ForeColor="White" />
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
                SelectCommand="SELECT [PRECALCUL].ORDRE, NOTE, ECARTTYPE, CASE WHEN NOTE >= [OR] THEN 'OR' WHEN NOTE >= [ARGENT] AND NOTE < [OR] THEN 'ARGENT' WHEN NOTE >= [BRONZE] AND NOTE < [ARGENT] THEN 'BRONZE' ELSE '' END AS MEDAILLE, [PRECALCUL].JURY, JURE1, JURE2, JURE3, JURE4, JURE5, [PRECALCUL].JOUR, [PRECALCUL].SERIE, [PRECALCUL].RANG, [PRECALCUL].CATEGORIE, [PRECALCUL].PAYS, [PRECALCUL].APPELLATION, [PRECALCUL].MILLESIME, [PRECALCUL].FIRME, [PRECALCUL].NOM, [PRECALCUL].PRENOM, [PRECALCUL].[CRU OU MARQUE], [PRECALCUL].CAB, [PRECALCUL].[NUMERO OXY], [PRECALCUL].VOLUME, [PRECALCUL].CEPAGE FROM [PRECALCUL], SEUIL_BY_CATEG SEUIL, LISTE_VINS LV  WHERE [PRECALCUL].CATEGORIE = SEUIL.CATEGORIE  AND [PRECALCUL].ORDRE = LV.ORDRE AND ((FIJEV IS NOT NULL AND 'FIJEV'=@PRIX) OR (HN_MALBEC IS NOT NULL AND 'HN_MALBEC'=@PRIX) OR (HS_MALBEC IS NOT NULL AND 'HS_MALBEC'=@PRIX) OR (HN_CAB_SAUV IS NOT NULL AND 'HN_CAB_SAUV'=@PRIX) OR (HS_CAB_SAUV IS NOT NULL AND 'HS_CAB_SAUV'=@PRIX) OR (SOMMELIER IS NOT NULL AND 'SOMMELIER'=@PRIX) OR (VINOFED IS NOT NULL AND 'VINOFED'=@PRIX) OR (DUAD IS NOT NULL AND 'DUAD'=@PRIX)) ORDER BY [NOTE] DESC"
                ...
                <SelectParameters>
                    <asp:QueryStringParameter Name="PRIX" QueryStringField="PRIX" Type="String" />
                </SelectParameters>
            </asp:SqlDataSource>
        </div>
    </form>
